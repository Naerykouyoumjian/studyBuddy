...
// schedules deadline email notification
async function deadlineEmail(firstName, taskId, listName, email, taskDescription, deadline, offset, newRequest){
    ...
    const job = cron.schedule(cronTime, async () =>{
        ...
        try{
            const response = await fetch("http://localhost:3001/delete-deadline-job", {
                method: "DELETE",
    ...
    if(newRequest){
        try{
            const response = await fetch("http://localhost:3001/save-deadline-job", {
                method: "POST",
                headers: {"Content-Type" : "application/json"},
                body: JSON.stringify({
                    taskId,
...
emailServer.post("/reschedule-email", async(req, res) =>{
    ...
    //let notificationDate = moment(deadline).subtract(deadlineOffset, "days").startOf("day").add(8, "hours").toDate();
    deadlineEmail(firstName, taskId, listName, email, taskDescription, deadline, offset, true);
...
const init = async () =>{
    ...
    try{
        const response = await fetch("http://localhost:3001/get-scheduled-jobs", {
            method: "GET",
            headers: {"Content-Type" : "application/json"},
        });
        const result = await response.json();
        if(result.success){
            console.log("scheduled jobs received from the database");
